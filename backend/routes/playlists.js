...
router.post("/my/:id/share", requireAuth, isPlaylistOwner, async (req, res) => {
  try {
    ...
    const playlist = req.playlist; // redan hämtad av isPlaylistOwner

    if (playlist.sharedWith.includes(userToShare._id)) {
      return res
        .status(409)
        .json({ error: "Playlist already shared with this user" });
    }

    playlist.sharedWith.push(userToShare._id);
    await playlist.save();
...
router.get("/shared-with-me", requireAuth, async (req, res) => {
  try {
    ...
    console.error("Shared with me failed:", err.message);
    res.status(500).json({ error: err.message });
  }
});
